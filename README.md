...
- **Typed Field / Param Overrides**: Use `<typed-field>` and `<typed-param>` to retype individual struct fields or function parameters at a specific C# type (typically a `[Flags]` enum), so call sites can drop the manual `(uint)` casts that the raw-`uint` C type would otherwise force
