...
class TestRemovalFunctionality:
    ...
    def test_simple_function_removal(self, temp_dir):
        ...
        header_library_pairs, namespace, include_dirs, renames, removals = parse_config_file(str(config))
        ...
        assert "remove_function" not in result
        # Verify kept functions are present
        assert "keep_function" in result
    ...
    def test_regex_function_removal(self, temp_dir):
        ...
        assert "SDL_Init" not in result
        assert "SDL_Quit" not in result
        assert "SDL_CreateWindow" not in result
    ...
    def test_struct_removal(self, temp_dir):
        ...
        assert "struct RemoveStruct" not in result
        # Verify kept structs are present
        assert "struct KeepStruct" in result
        assert "struct AnotherKeep" in result
    ...
    def test_enum_removal(self, temp_dir):
        ...
        assert "enum RemoveEnum" not in result
        assert "REMOVE_A" not in result
        # Verify kept enum is present
        assert "enum KeepEnum" in result
        assert "KEEP_A" in result
    ...
    def test_union_removal(self, temp_dir):
        ...
        assert "RemoveUnion" not in result
        # Verify kept union is present
        assert "KeepUnion" in result
    ...
    def test_regex_removal_complex_pattern(self, temp_dir):
        ...
        assert "internal_helper_function" not in result
        assert "_private_function" not in result
        assert "__system_function" not in result
